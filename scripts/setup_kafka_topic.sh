...
# decision-service publishes to when it gives up on a message, and the
# waiting-room-joins topic (Phase 10) that waiting-room-api publishes
# to and risk-service consumes. Run this before starting any of
# waiting-room-api, checkout-api, decision-service, or risk-service for
# the first time, and again any time you want a clean slate (e.g. to
# clear out Phase 5's manual test data or fix a topic that got
# auto-created with the wrong partition count).
#
# Usage: scripts/setup_kafka_topic.sh [partitions]
#   scripts/setup_kafka_topic.sh        # main topics default to 3 partitions
...
create_topic "$WAITING_ROOM_JOINS_TOPIC" "$MAIN_PARTITIONS"
...
docker exec "$CONTAINER" /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server "$BOOTSTRAP" --describe --topic "$WAITING_ROOM_JOINS_TOPIC"
